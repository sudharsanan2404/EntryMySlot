...
-- Creates the canonical tables that the rest of the migrations extend.
-- Idempotent: every CREATE uses IF NOT EXISTS so this is safe to apply
-- on fresh databases (cold start) or to be a no-op (already exists).
--
-- Note: every column/index/constraint is idempotent so that migrations
-- 001+ can safely re-run this on already-bootstrapped databases.
-- ============================================================================

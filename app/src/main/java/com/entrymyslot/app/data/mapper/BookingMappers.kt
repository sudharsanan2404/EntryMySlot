...
internal fun bookingStatus(status: String?, date: String?): BookingStatus? = when (status?.lowercase()) {
    ...
    "confirmed" -> if (runCatching { Instant.parse(date).isBefore(Instant.now()) }.getOrDefault(false)) BookingStatus.COMPLETED else BookingStatus.UPCOMING

...
internal fun ApiMovie.toUi() = id?.takeIf(String::isNotBlank)?.let { key ->
    Movie(key, title.orEmpty(), releaseDate.orEmpty(), "", "", posterUrl, backdropUrl,
...
internal fun ApiEvent.toUi() = id?.takeIf(String::isNotBlank)?.let { key ->
    val minimum = ticketTiers.orEmpty().mapNotNull { it.price }.minOrNull()
        ?: zones.orEmpty().mapNotNull { it.price?.movePointRight(2)?.intValueExact() }.minOrNull()
    Event(key, title.orEmpty(), startDate.orEmpty().substringBefore('T'), listOfNotNull(venue, city).joinToString(", "),
        (price?.toPlainString()?.let { "₹$it" } ?: minimum?.let { "₹${java.math.BigDecimal(it).movePointLeft(2).toPlainString()}" }).orEmpty(), posterUrl ?: bannerUrl, description.orEmpty(), category.orEmpty(),
...
internal fun Venue.toUi() = id?.takeIf(String::isNotBlank)?.let { key ->
    ...
    Turf(key, name.orEmpty(), "", listOfNotNull(address, city).joinToString(", "),
